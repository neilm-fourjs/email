...
CONSTANT VER = "UNK" CONSTANT PRG = "libMail" CONSTANT PRGDESC = "4gl/Java Mail Api" CONSTANT PRGAUTH = "Neil J.Martin" -- Flag for if processed. Becomes xxx00000yy where yy=head_key from insert.
    CONSTANT MYFLAG = "fjs" CONSTANT GUI = 0 CONSTANT STRINGLIMIT = 1000000 CONSTANT c_debug BOOLEAN = FALSE CONSTANT saveAttachments BOOLEAN = TRUE
&ifdef DBG
...
-- Open the default Folder
	...
	IF use_db THEN -- replace folder list
		...
		FOR i = 1 TO mboxes.getLength()
			LET mbox.* = mboxes[i].*
			INSERT INTO imapi_folders
					VALUES(l_user, mbox.name, mbox.fullname, mbox.img, mbox.id, mbox.pid,
							0, 0)
		END FOR
	END IF
	LET online = TRUE
...
-- try to open read/write and IF that fails try read-only
	...
	CALL mailStatus(
			__LINE__, "Mails " || totalMessages || "(" || newMessages || ")")
	RETURN TRUE
...
-- check for and remote null to,cc,bcc array elements.
		...
		FOR i = i2 TO 1 STEP -1
			IF smtp_to[i] IS NULL THEN
				CALL smtp_to.deleteElement(i)
			END IF
		END FOR
		LET i2 = smtp_cc.getLength()
		FOR i = i2 TO 1 STEP -1
			IF smtp_cc[i] IS NULL THEN
				CALL smtp_cc.deleteElement(i)
			END IF
		END FOR
		LET i2 = smtp_bcc.getLength()
		FOR i = i2 TO 1 STEP -1
			IF smtp_bcc[i] IS NULL THEN
				CALL smtp_bcc.deleteElement(i)
			END IF
		END FOR
...
--		MAILDEBUG(3,i||" "||folders[i].getFullName())
		...
		IF x > 0 THEN
			LET nam = nam.subString(x + 1, nam.getLength())
		END IF
		LET mboxes[idx].name = nam
		CASE UPSHIFT(nam)
			WHEN "[GMAIL]"
				LET mboxes[idx].img = D_FLD_GMAIL
			WHEN "INBOX"
				LET mboxes[idx].img = D_FLD_INBOX
			WHEN "JUNK"
				LET mboxes[idx].img = D_FLD_JUNK
			WHEN "SPAM"
				LET mboxes[idx].img = D_FLD_JUNK
			WHEN "TRASH"
				LET mboxes[idx].img = D_FLD_DELETED
			WHEN "SENT"
				LET mboxes[idx].img = D_FLD_SENT
			WHEN "SENT MAIL"
				LET mboxes[idx].img = D_FLD_SENT
		END CASE
...
-- Remove all my tags while testing!!
	...
	IF use_db AND mno > 0 THEN
		IF mailDBGetHead(m_mbox, mno, mail_key) THEN
			RETURN
		END IF
		IF flg_fjs IS NOT NULL THEN
			CALL flags.remove(
					flg_fjs) -- because flag must be wrong because record not found in db.
			CALL m_folder.setFlags(mno, mno, flags, FALSE)
		END IF
	END IF
...
-- FROM
	...
	IF a IS NOT NULL THEN
		...
		END FOR
		IF mno > 0 THEN
			LET mails[mno].from_whom = a[1].toString()
		END IF
	END IF

	IF mno > 0 THEN
		CALL recps[mno].recp.clear()
	END IF
-- REPLY TO
	IF ((a = m.getReplyTo()) IS NOT NULL) THEN
		FOR j = 1 TO a.getLength()
			...
			IF mno > 0 THEN
				IF j = 1 THEN
					LET mails[mno].reply_to = a[1].toString()
				END IF
...
-- TO
	...
	IF a IS NOT NULL THEN
		FOR j = 1 TO a.getLength()
			...
			IF mno > 0 THEN
				IF j = 1 THEN
					LET mails[mno].to_whom = a[1].toString()
				END IF
...
-- fetching the actual content data until we need it.
	...
	IF (p.isMimeType("message/rfc822")) THEN
		...
		LET m_level = m_level + 1
		IF mailRetvPart(CAST(p.getContent() AS javax.mail.Part), mno, hk, pno + 1)
				THEN
		END IF
		LET m_level = m_level - 1
		LET done = TRUE
...
-- many mailers don't include a Content-Disposition
		IF {(disp IS NULL OR disp.equalsIgnoreCase(Part.ATTACHMENT)) AND} saveAttachments
				THEN
			CALL mailSaveAtt( filename, hk, CAST(p AS MimeBodyPart))
					RETURNING localName
			IF mno > 0 THEN
				LET mails[mno].flg_attach = D_ATTACH
			END IF
		ELSE
...
PUBLIC FUNCTION mailDBcreateTables()
	...
	TRY
		CREATE TABLE imapi_folders(
				account VARCHAR(50), dispname VARCHAR(40), origname VARCHAR(60),
				icon VARCHAR(20), id SMALLINT, pid SMALLINT, no_mails INTEGER,
				unread INTEGER)
	...
	CALL drop_tab(tname)
	TRY
		CREATE TABLE imapi_mail_header(head_key SERIAL, D_EMAILHEADER)
		MAILDEBUG(3, "Created:" || tname)
	CATCH
		MAILDEBUG(-1, "ERR:Create '" || tname || "' failed:" || SQLERRMESSAGE)
	END TRY
	...
	TRY
		CREATE TABLE imapi_mail_recp(
				head_key INTEGER, mode VARCHAR(10), email_addr VARCHAR(200))
		MAILDEBUG(3, "Created:" || tname)
	CATCH
		MAILDEBUG(-1, "ERR:Create '" || tname || "' failed:" || SQLERRMESSAGE)
	END TRY
	...
	CALL drop_tab(tname)
	TRY
		CREATE TABLE imapi_mail_bodypart(body_key SERIAL, D_EMAILBODYPART)
		MAILDEBUG(3, "Created:" || tname)
	CATCH
		MAILDEBUG(-1, "ERR:Create '" || tname || "' failed:" || SQLERRMESSAGE)
	END TRY
...
-- Fetch the folder list
	DECLARE f CURSOR FOR
			SELECT * FROM imapi_folders WHERE account = l_user ORDER BY id, pid
...
FUNCTION mailDBretv(mno)
	...
	DECLARE mbc CURSOR FOR
			SELECT * FROM imapi_mail_bodypart
					WHERE head_key = mh.head_key AND mime_type[1, 4] = "TEXT"
					ORDER BY part_no
	FOREACH mbc INTO mb.*
		IF mb.mime_type IS NULL THEN
			LET mb.mime_type = "(null)"
		END IF
...
FUNCTION mailDBFetch(mbox)
	...
	DECLARE mf CURSOR FOR
			SELECT * FROM imapi_mail_header
					WHERE account = m_user AND foldername = mbox ORDER BY message_no
	LET mno = 0
	FOREACH mf INTO mh.*
		LET mno = mno + 1
...
FUNCTION mailDBSetHead(mno, mh)
	...
	IF mh.flg_answered = 1 THEN
		LET mails[mno].flg_answered = D_ANSWERED
	END IF
	IF mh.flg_answered = 2 THEN
		LET mails[mno].flg_answered = D_FORWARDED
	END IF
	IF mh.flg_answered = 3 THEN
		LET mails[mno].flg_answered = D_FORW_ANSW
	END IF
	IF mh.flg_junk THEN
		LET mails[mno].flg_junk = D_JUNK
	END IF
	IF mh.flg_attach THEN
		LET mails[mno].flg_attach = D_ATTACH
	END IF
...
{ This is not possible because a reply can match a previous reply!!!
	...
	IF mails[mno].flg_answered = D_ANSWERED THEN
		LET mh.flg_answered = 1
	END IF
	IF mails[mno].flg_answered = D_FORWARDED THEN
		LET mh.flg_answered = 2
	END IF
	IF mails[mno].flg_answered = D_FORW_ANSW THEN
		LET mh.flg_answered = 3
	END IF
	IF mails[mno].flg_junk IS NOT NULL THEN
		LET mh.flg_junk = TRUE
	END IF
	IF mails[mno].flg_attach IS NOT NULL THEN
		LET mh.flg_attach = TRUE
	END IF
	...
	ELSE
		CALL sb_emailHTMLBody.clear()
	END IF
	IF NOT mailRetvPart(m, mno, mh.head_key, 0) THEN
		RETURN
	END IF -- Fetch mail body.
	...
	UPDATE imapi_mail_header SET (flg_attach, content_type)
			= (mh.flg_attach, mh.content_type) WHERE head_key = mh.head_key
