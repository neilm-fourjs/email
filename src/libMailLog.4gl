...
PUBLIC FUNCTION logDebug( line, file, lev, msg )
	...
	IF m_debugLevel IS NULL THEN LET m_debugLevel = 3 END IF
	IF lev > m_debugLevel THEN RETURN END IF

	LET l_time = TIME
	LET file = os.path.basename( file )
	LET file = os.path.rootname( file )
	IF msg IS NULL THEN LET msg = "(null)" END IF
	...
	IF m_debugfile THEN
		IF m_logName IS NULL OR m_logName.getLength() < 2 THEN
			LET m_logName = YEAR(TODAY)||(MONTH(TODAY) USING "&&")||(DAY(TODAY) USING "&&")||l_time[1,2]||l_time[4,5]
		END IF
		IF m_debugOut IS NULL THEN
			LET m_debugOut = base.channel.create()
			IF m_path IS NULL THEN LET m_path = "debug"||os.path.separator() END IF
		END IF
		TRY
