...
#+
#+ @return true/false
FUNCTION sendmail( l_recipients, l_attachements, l_subject, l_mailtext, l_texttype, l_confirm ) --{{{
	DEFINE l_recipients DYNAMIC ARRAY OF RECORD mode STRING, recipient STRING END RECORD
	...
	FOR x = 1 TO l_recipients.getLength()
		CASE UPSHIFT(l_recipients[x].mode )
			...
			WHEN "BCC"
				LET libMail.smtp_bcc[ smtp_bcc.getLength() + 1 ] = l_recipients[x].recipient
		END CASE
	END FOR
	FOR x = 1 TO l_attachements.getLength()
		IF l_attachements[x].getLength() > 1 THEN
			LET libMail.smtp_attachements[ libMail.smtp_attachements.getLength() + 1 ] = l_attachements[x]
		END IF
	END FOR
